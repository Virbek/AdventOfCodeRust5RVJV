mod d1;
mod d2;
mod d3;
mod d4;
mod d5;
mod d6;
mod d7;
mod d8;
mod d9;
mod d10;
mod d11;

fn main() {
    //Day 1
    println!("Day 1 - ver1 - part1, le mot de passe est: {}", d1::v1::solve_part1(d1::INPUT));
    println!("Day 1 - ver2 - part1, le mot de passe est: {}", d1::v2::solve_part1(d1::INPUT));

    println!("Day 1 - ver1 - part2, le mot de passe est: {}", d1::v1::solve_part2(d1::INPUT));
    println!("Day 1 - ver2 - part2, le mot de passe est: {}", d1::v2::solve_part2(d1::INPUT));

    println!("Day 1 - ver3 - part1, le mot de passe est: {}", d1::v3::solve_part1(d1::INPUT));
    println!("Day 1 - ver3 - part2, le mot de passe est: {}", d1::v3::solve_part2(d1::INPUT));

    //Day 2
    println!("Day 2 - ver1 - part1, la somme des invalides est: {}", d2::v1::solve_part1(d2::INPUT));
    println!("Day 2 - ver2 - part1, la somme des invalides est: {}", d2::v2::solve_part1(d2::INPUT));

    println!("Day 2 - ver1 - part2, la somme des invalides est: {}", d2::v1::solve_part2(d2::INPUT));
    println!("Day 2 - ver2 - part2, la somme des invalides est: {}", d2::v2::solve_part2(d2::INPUT));

    //Day 3
    println!("Day 3 - ver1 - part1, le résultat: {}", d3::v1::solve_part1(d3::INPUT));
    println!("Day 3 - ver2 - part1, le résultat: {}", d3::v2::solve_part1(d3::INPUT));
    println!("Day 3 - ver3 - part1, le résultat: {}", d3::v3::solve_part1(d3::INPUT));

    println!("Day 3 - ver1 - part2, le résultat: {}", d3::v1::solve_part2(d3::INPUT));
    println!("Day 3 - ver2 - part2, le résultat: {}", d3::v2::solve_part2(d3::INPUT));

    //Day 4
    println!("Day 4 - ver1 - part1, le résultat: {}", d4::v1::solve_part1(d4::INPUT));
    println!("Day 4 - ver1 - part2, le résultat: {}", d4::v1::solve_part2(d4::INPUT));

    //Day 5
    println!("Day 5 - ver1 - part1, le résultat: {}", d5::v1::solve_part1(d5::INPUT));
    println!("Day 5 - ver1 - part2, le résultat: {}", d5::v1::solve_part2(d5::INPUT));
    println!("Day 5 - ver2 - part1, le résultat: {}", d5::v2::solve_part1(d5::INPUT));
    println!("Day 5 - ver2 - part2, le résultat: {}", d5::v2::solve_part2(d5::INPUT));
    println!("Day 5 - ver3 - part1, le résultat: {}", d5::v3::solve_part1(d5::INPUT));
    println!("Day 5 - ver3 - part2, le résultat: {}", d5::v3::solve_part2(d5::INPUT));
  
    //Day 6
    println!("Day 6 - ver1 - part1, le résultat: {}", d6::v1::solve_part1(d6::INPUT));
    println!("Day 6 - ver2 - part1, le résultat: {}", d6::v2::solve_part1(d6::INPUT));

    println!("Day 6 - ver1 - part2, le résultat: {}", d6::v1::solve_part2(d6::INPUT));
    println!("Day 6 - ver2 - part2, le résultat: {}", d6::v2::solve_part2(d6::INPUT));

    //Day 7
    println!("Day 7 - ver1 - part1, le résultat: {}", d7::v1::solve_part1(d7::INPUT));
    println!("Day 7 - ver1 - part2, le résultat: {}", d7::v1::solve_part2(d7::INPUT));

    println!("Day 7 - ver2 - part1, le résultat: {}", d7::v2::solve_part1(d7::INPUT));
    println!("Day 7 - ver2 - part2, le résultat: {}", d7::v2::solve_part2(d7::INPUT));

    println!("Day 7 - ver3 - part1, le résultat: {}", d7::v3::solve_part1(d7::INPUT));
    println!("Day 7 - ver3 - part2, le résultat: {}", d7::v3::solve_part2(d7::INPUT));
    //Day 8
    println!("Day 8 - ver1 - part1, le résultat: {}", d8::v1::solve_part1(d8::INPUT));
    println!("Day 8 - ver1 - part2, le résultat: {}", d8::v1::solve_part2(d8::INPUT));

    println!("Day 9 - ver1 - part1, le résultat: {}", d9::v1::solve_part1(d9::INPUT));
    println!("Day 9 - ver1 - part2, le résultat: {}", d9::v1::solve_part2(d9::INPUT));

    println!("Day 9 - ver2 - part1, le résultat: {}", d9::v2::solve_part1(d9::INPUT));

    println!("Day 10 - ver1 - part1, le résultat: {}", d10::v1::solve_part1(d10::INPUT));

    println!("Day 11 - ver1 - part1, le résultat: {}", d11::v1::solve_part1(d11::INPUT));
    println!("Day 11 - ver1 - part2, le résultat: {}", d11::v1::solve_part2(d11::INPUT));

    println!("Day 11 - ver2 - part1, le résultat: {}", d11::v2::solve_part1(d11::INPUT));
    println!("Day 11 - ver2 - part1, le résultat: {}", d11::v2::solve_part2(d11::INPUT));
}