...
pub fn solve_part1(input: &str) -> u32{
    ...
    for (i, c) in input.chars().enumerate(){
        if c == '-' {
            ...
            if count == 1 {
                ...
                let x: u64 = save_first.trim().parse().unwrap();
                let y: u64 = save_last.trim().parse().unwrap();
    ...
    let reste: String = input.split_at(pos).1.to_string();
    ...
    for c in reste.chars() {
        ...
        if c == '\n' {
            if !save.is_empty(){
                //println!("save : {}", save);
                let x: u64 = save.trim().parse().unwrap();
                contenu.push(x);
                save.clear();
            }
        }else{
